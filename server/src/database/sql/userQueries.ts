...
export const insertNewUserQuery = (user: User) : string => {
...
export const selectUserLogInQuery = (username: string, password: string) : string => {
    return /*sql*/`SELECT id FROM user WHERE username='${username}' AND hash_password='${password}'`
}

export const selectOneUsernameQuery = (username: string) : string => {
    return /*sql*/`SELECT 1 from user WHERE username='${username}'`
}

export const selectProfileQuery = (id: number) : string => {
...
export const selectProfilePictureQuery = (id: number) : string => {
    return /*sql*/`SELECT profile_pic FROM user WHERE user.id = ${id}`
}

export const selectProfileFullQuery = (id:number) : string => {
...
export const updateProfileQuery = (p: Profile) : string => {
